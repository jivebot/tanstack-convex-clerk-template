...
1. Set the `VITE_CLERK_PUBLISHABLE_KEY` in your `.env.local`

2. Enable **Organizations** in Clerk Dashboard > Configure > Organization management

3. Create a **Convex** JWT template in Clerk Dashboard > Configure > JWT Templates:
   - Select the "Convex" template
   - Add these custom claims to include organization data:
     ```json
     {
       "org_id": "{{org.id}}",
       "org_role": "{{org.role}}",
...
4. Set the JWT issuer in Convex:
   ```bash
